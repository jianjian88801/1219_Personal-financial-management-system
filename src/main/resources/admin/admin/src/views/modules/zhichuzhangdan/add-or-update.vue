    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="账单类型" prop="zhangdanleixing">
          <el-select v-model="ruleForm.zhangdanleixing" placeholder="请选择账单类型">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="支出方式" prop="zhichufangshi">
          <el-select v-model="ruleForm.zhichufangshi" placeholder="请选择支出方式">
            <el-option
                v-for="(item,index) in zhichufangshiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="支出方式" prop="zhichufangshi">
	      <el-input v-model="ruleForm.zhichufangshi"
                placeholder="支出方式" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhichujine = true;
          ...
          if(o=='zhichufangshi'){
            this.ruleForm.zhichufangshi = obj[o];
	    this.ro.zhichufangshi = true;
            continue;
          }
          if(o=='beizhu'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.zhichufangshiOptions = "现金支付,转账支付".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `zhichuzhangdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhichuzhangdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhichuzhangdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
