<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="事件名称" prop="shijianmingcheng">
          <el-input v-model="ruleForm.shijianmingcheng" 
              placeholder="事件名称" clearable  :readonly="ro.shijianmingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="事件名称" prop="shijianmingcheng">
              <el-input v-model="ruleForm.shijianmingcheng" 
                placeholder="事件名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="完成情况" prop="wanchengqingkuang">
          <el-select v-model="ruleForm.wanchengqingkuang" placeholder="请选择完成情况">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="完成情况" prop="wanchengqingkuang">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="提醒日期" prop="tixingriqi">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tixingriqi" label="提醒日期" prop="tixingriqi">
              <el-input v-model="ruleForm.tixingriqi" 
                placeholder="提醒日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="提醒事宜" prop="tixingshiyi">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="提醒事宜"
                  v-model="ruleForm.tixingshiyi" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tixingshiyi" label="提醒事宜" prop="tixingshiyi">
                    <span>{{ruleForm.tixingshiyi}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='shijianmingcheng'){
            this.ruleForm.shijianmingcheng = obj[o];
	    this.ro.shijianmingcheng = true;
            continue;
          }
          if(o=='tixingshiyi'){
            this.ruleForm.tixingshiyi = obj[o];
	    this.ro.tixingshiyi = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.wanchengqingkuangOptions = "是,否".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `beiwanglu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `beiwanglu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.beiwangluCrossAddOrUpdateFlag = false;
                  this.parent.search();
