<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="收入方式" prop="shourufangshi">
          <el-select v-model="ruleForm.shourufangshi" placeholder="请选择收入方式">
            <el-option
                v-for="(item,index) in shourufangshiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="收入方式" prop="shourufangshi">
	      <el-input v-model="ruleForm.shourufangshi"
                placeholder="收入方式" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shourujine = true;
          ...
          if(o=='shourufangshi'){
            this.ruleForm.shourufangshi = obj[o];
	    this.ro.shourufangshi = true;
            continue;
          }
          if(o=='beizhu'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.shourufangshiOptions = "现金收入,转账收入".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shouruzhangdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shouruzhangdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shouruzhangdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
